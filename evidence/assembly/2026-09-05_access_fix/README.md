...
| Knee M3 pockets | All five mouths and seating envelopes clear. Install before stop plate/encoder; exact owned M3 coupon remains required. |
| Shoulder plate / cover | Four insert mouths clear; four screws enter outboard. Cover contains no inserts. Fit housing screws before the cover, and reverse that order for service. |
...
| Mode A stand | Five blind M3 receivers have open installation paths from the detached mount face. Install inserts before attaching the panel, and attach the panel before fitting post A, which covers one panel-bolt approach. Bench access retains its separate checked mount-face orientation. |
...
| Cartridge eyes, guide rod, shims, bumper | No additional heat-set receivers. Existing motion checks remain geometric only. The received 150 mm spring is incompatible with the specified cartridge and stays out of the build. |
