...
the CAD. When the order arrives, only check that the pieces are undamaged and
perform the hand-fit steps below. A pin that needs drilling, filing, hammering,
or screw pull-down is a failed interface, not a request to rework the hardware.
...
3. Support the shoulder hub's flange flat. Start three Ø4 × 10 dowels straight
   into its Ø4.05 blind sockets using a smooth-jaw vise or controlled arbor
   press, never a hammer. Seat each 5.0 mm deep. Stop if the hub whitens,
   splits, or a pin cocks. Offer the proximal root over the exposed halves;
...
5. Complete the unchanged cartridge, guide, spring, D10 knee-pin keeper, and
   ...
   and seat the cotter fully without bending or reshaping it. Both integral printed lands provide the designed
   34.0 mm retained stack; add no spacer.
...
The ordered hardware remains `PHYSICAL ASSEMBLY PENDING` until these binary
hand-fit checks pass. This does not require calipers and does not authorize
powered motion, ground contact, added mass, or structural testing.

## Verification record

Fusion verified the exact 40 mm clevis insertion paths, washer/cotter space,
the captive Ø6 stop stack, the three shoulder-root dowel locations, the
unchanged calibrated print faces, and all -8°…+15° hand poses. Every STL is a
closed manifold with zero non-manifold edges and zero degenerate triangles.
See the [release evidence](../../evidence/assembly/2026-09-21_ordered_pin_integration/)
