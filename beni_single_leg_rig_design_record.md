...
`rig_export.py` (STL), `manufacturing/stl_inspect.py` (mesh measurement).
...
are kept under `archive_laser/` only in case the two-leg build wants the steel arc
back.
...
|---|---|---|---|
| 1 | MGN12 rail **300 mm**, "231 mm usable travel" | **400 mm** | 231 mm is the *one-block* figure. Two MGN12H at 80 mm centres occupy 45.4 + 80 = **125.4 mm** of rail, so 300 mm leaves 150.6 mm after bumpers against a **151.5 mm** stroke requirement — 0.9 mm short before any bumper. 400 mm gives 250.6 mm. §2.1 |
| 2 | Passive drop limit **49 mm** | **46.3 mm** to the step-10 gate | The brief's own step-10 gate is φ_peak < +24°. That is reached at **46.3 mm**, so a 49 mm drop already breaks the gate. +25° is at 50.7 mm, the +27° metal stop at **60.0 mm**. §3 |
| 3 | Peak force **~53 N** | **54.8 N** at the stop, 51.4 N at +25° | Exact integration of the frozen spring curve. §3 |
| 4 | "Print the two GAUGE coupons" resolves C2 and C3 | Resolves **C3 only** | `GAUGE_Shoulder_Motor_Interface.stl` is **9.5 mm long** (y −6…+3.5) — the front face and the output interface, nothing else. It cannot see a 40-vs-44 mm motor. §2.4 |
| 5 | Ballast adjustable for **1.2** / 1.645 / 2.0 kg | **1.2 kg is impossible** | The bare slide is **1607.6 g**. 1.2 kg would need 408 g *removed* and there is nothing left to remove. §5 |
| 6 | `Chassis_Shoulder_Plate_L` is "Ø96, 8 × M3 on Ø74 PCD" | 120 × 120 panel, Ø48 bore | It is the chassis **side panel**. The Ø74 PCD is the *motor's* pattern, already used by the motor's own screws, so the carriage bolts to the panel's **five existing frame-bolt holes** instead. §2.2 |
| 7 | (not mentioned) | Housing screws **M3 × 8**, not M3 × 10 | The inherited model uses M3 × 10 through a 5 mm panel into a **4.0 mm** thread → 5 mm of screw into 4 mm of hole. Bottoms out before it clamps — the same defect class as the two found in design record §7 [REV2]. §9 |
| 8 | `Knee_Stop_Arc_L`, laser-cut 3 mm steel at 45 HRC | **Deleted.** The +27° stop becomes a **compression stack inside the spring cartridge**; a printed plate keeps the −8° stop and a +28° backup | No laser parts. And the steel plate only worked because its slot ends were *conformal*; every printed or bought convex substitute reverts to Hertzian line contact at 1.0–1.8 GPa. §8 |
| 9 | `Distal_Link_L` "reuse as-is" | **Re-exported** | Adopting §2.3 moves the sleeve's Ø16 bore into the printed link as Ø10. This is the hidden cost of deleting the double-D. §4 |
| 10 | (not mentioned) | The cartridge's usable internal length is **44.570 mm consumed**, not 35.57 | Measured with a probe ring at six knee angles, spread 0.0000 mm. The dead-length build-up (11.00 + 14.57 + 4.0 + 6.0) is 9.0 mm optimistic. Sizing the new stop from the build-up would have put the hard stop at **+10°**. §8 |
...
| Part | What it is | Route |
|---|---|---|
| `RIG_Base` | 2020 frame, 400 × 300 | buy extrusion |
| `RIG_Column` | 2020 × 480 vertical | buy |
| `RIG_Braces` | 2 × 2020 diagonals, 191 + 219 mm | buy |
| `RIG_Rail` | **MGN12, 400 mm** + 2 × MGN12H | buy |
| `RIG_Carriage` | 8 mm PA-CF, 154 × 170 | **print** |
| `RIG_Index_Post` | 2020 × 405, forward of the motor | buy |
| `RIG_Index_Bar` | 12.5 mm PA-CF, 17 stations at 10 mm | **print** |
| `RIG_Mode_Pin` | Ø8 quick-release pin | buy |
| `RIG_Hard_Stops` | 2 × Ø14 PU bumpers | buy |
| `RIG_Ballast_Pot` × 2 | printed cup, 344 g of shot | **print** + bought shot |
| `RIG_Knee_Stop_Plate_L` | printed arc, −8° stop + backup | **print** |
| `HW_WasherStack_M5` + `RIG_Knee_Bumper_Tube_L` | the +27° hard stop | buy + **print (TPU)** |
| `RIG_Torque_Arm` | 200 mm lever, 12 mm PA-CF | **print** |
| `RIG_Scale_Pedestal` | 2 × 2020 + shelf | buy |
| `RIG_Floor_Plate` | 260 × 60 × 6 | **print** or 6 mm alu |
| `RIG_Cable_Post_A/B` | two service-loop anchors | **print** |
| `RIG_Knee_Collar_L`, `RIG_Knee_Magnet_Carrier_L` | §2.3 substitutes | **print** |
...
cartridge and the spring. Reused **modified**: `Distal_Link_L` (§4).
...
The stack-up reproduces the brief exactly:
...
case still sits at **fs 4.40**; one block would be 2.20, below spec.

### 2.4 What the GAUGE coupons actually resolve

Measured out of the STLs with `manufacturing/stl_inspect.py`:
...
**Take 45 mm as the passive planning limit, not 49 mm.** The brief's own gate is
φ_peak < +24°, and 49 mm exceeds it. The brief's conclusion is otherwise
confirmed and strengthened: a 100 mm free drop bottoms the knee out, and it does
so from 60 mm upward.
...
element in the load path and it reaches metal at +27°.
...
| 3 | Wheel clears the floor through the whole Mode B travel | **PASS** — see below |
| 4 | Mass properties incl. sprung/unsprung | **done**, §5 |
| 5 | Torque arm cannot hit the column | **PASS** — Y bands disjoint by 38.5 mm, so it cannot at *any* angle |
| 6 | Re-verify the 63 mm stack-up | **PASS** — 63.00 mm, matches |
...
3. **`Sketch.saveAsDXF` after `projectCutEdges` mirrored the hole pattern.** It
   ...
   trusted without an independent area check (see `archive_laser/`).
...
| **Bench clamps** | 2 | the brief is right: it walks otherwise |
...
| M4 heat-set inserts, **5.8 long** | 6 + 4 | hub root joint + ballast studs; an 8 mm insert breaks through the 8 mm flange |
| M4 threaded stud, 30 mm | 4 | ballast |
...
| Ø8 quick-release pin, 20 mm grip | 1 | mode pin / drop release |
| Ø19/Ø13.6 × 0.5 shim washers | 8 + 8 | cartridge preload |
| M3 steel washers | 6 | under every wheel-hub head |
| M12 washers (~7 g) | 20 | ballast trim |
| 6800-2RS bearing | 2 | |
| Ø4 × 32 clevis pin + E-clip DIN 6799-4 | 2 | |
| Ø6 × 2.5 diametric NdFeB magnet | 1 | |
| PU bumper, Ø14 | 2 | rail travel stops |
| **Steel shot / airgun BBs / M4 nuts** | ~150 g | ballast fill, §5. Anything dense and granular |
| Main spring, Ø19 × 2.6 × 55, chrome-silicon A877 | 1 | 10.45 N/mm ±5 % |

### Nothing to laser-cut and nothing to machine
Every part is printed or bought. The retired steel-arc DXFs are under
`archive_laser/` if the two-leg build wants them.

### Instruments
5 kg kitchen scale · dial indicator (the 0.05 TIR on the printed magnet carrier)
· pin gauges · milliohm meter · feeler gauges. **The kitchen scale is also how the
ballast is set** — weigh the shot into the pots.
...
**Stop at h ≈ 45 mm** until the landing controller is live (§3).

---

## 11. What this rig cannot test

- **Balance and pitch dynamics.** One leg on a vertical slide has no free body
  and no pitch degree of freedom.
